...
use vulkayes_core::{ash::vk, log, memory::device::naive::NaiveDeviceMemoryAllocator, prelude::*, resource::image::params::ImageViewRange};
...
impl ApplicationState {
	...
	fn create_depth_image_view(
		...
		let depth_image_view = {
			let depth_image = Image::new(
				...
				queue.into(),
				vulkayes_core::resource::image::params::ImageAllocatorParams::Some {
	...
	fn create_framebuffers<'a>(
		...
		let framebuffers: Vec<vk::Framebuffer> = present_image_views
			.map(|present_image_view| {
				let framebuffer_attachments = [*present_image_view.deref().deref(), *depth_image_view.deref().deref()];
	...
	fn check_and_recreate_swapchain(&mut self) {
		...
		if self.swapchain.outdated != 0 {
			...
			let new_data = self.swapchain.swapchain.recreate(
			...
			match new_data {
				...
				Ok(data) => {
					...
					self.swapchain.scissors = vk::Rect2D {
						...
						extent: vk::Extent2D { width: self.surface_size[0].get(), height: self.surface_size[1].get() }
					};
	...
	pub fn new_input_thread<T: ChildExampleState>(
		...
		Self::new_input_thread_inner(
			window_size,
			move |window, input_receiver, oneoff_sender| {
	...
	fn new(window: Window, input_receiver: mpsc::Receiver<InputEvent>, oneoff_sender: mpsc::Sender<Window>) -> Self {
		let surface_size = [NonZeroU32::new(window.inner_size().width).unwrap(), NonZeroU32::new(window.inner_size().height).unwrap()];

		let (base, surface) = Self::setup_base(
			IntoIterator::into_iter(vulkayes_window::winit::required_extensions(&window)),
			|instance| {
				let surface = vulkayes_window::winit::create_surface(instance, &window, Default::default()).expect("Could not create surface");
		...
		let swapchain = Self::setup_swapchain(
			surface,
			&base.present_queue,
			surface_size
		);
