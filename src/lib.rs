use std::{default::Default, ffi::CStr, num::NonZeroU32, ops::Deref, sync::mpsc};

// use winit::window::Window;
use ash::vk;
use vulkayes_core::{ash, log, memory::device::naive::NaiveDeviceMemoryAllocator, prelude::*};
...
pub const VALIDATION_LAYER: &'static CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"VK_LAYER_KHRONOS_validation\0") };

pub fn record_command_buffer(command_buffer: &Vrc<CommandBuffer>, f: impl FnOnce(&Vrc<CommandBuffer>)) {
	unsafe {
		{
...
pub fn submit_command_buffer_simple(command_buffer: &Vrc<CommandBuffer>, submit_queue: &Vrc<Queue>) {
...
impl ExampleBase {
	...
	pub fn with_input_thread(window_size: [NonZeroU32; 2], new_thread_fn: impl FnOnce(Self) + Send + 'static) {
	...
	fn draw(&mut self, f: &impl Fn(&mut Self, u32)) {
		...
		match self.present_queue.present(
	...
	fn new(window_size: [NonZeroU32; 2], window: &Window, input_receiver: mpsc::Receiver<InputEvent>) -> Self {
		...
		let layers = sub_release! {
			std::iter::once(VALIDATION_LAYER),
			std::iter::empty()
		};
		...
		let instance = Instance::new(
			...
			layers,
			IntoIterator::into_iter(vulkayes_window::winit::required_extensions(&window)).chain(std::iter::once(
		...
		let swapchain_create_info = Self::swapchain_create_info(&surface, &present_queue, window_size);
		...
		let depth_image_view = {
			let depth_image = Image::new(
				...
				ImageAllocatorParams::Some { allocator: &device_memory_allocator, requirements: vk::MemoryPropertyFlags::DEVICE_LOCAL },
				Default::default()
			)
		...
		record_command_buffer(
			...
			|command_buffer| {
				...
				let layout_transition_barriers = vk::ImageMemoryBarrier::builder()
					...
					.subresource_range(vk::ImageSubresourceRangeBuilder::from(depth_image_view.subresource_range()).build());
		...
		let present_complete_semaphore = Semaphore::binary(device.clone(), Default::default()).expect("Could not create semaphore");
		let rendering_complete_semaphore = Semaphore::binary(device.clone(), Default::default()).expect("Could not create semaphore");
